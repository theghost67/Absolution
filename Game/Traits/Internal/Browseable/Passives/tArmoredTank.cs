...
namespace Game.Traits
{
    ...
    public class tArmoredTank : PassiveTrait
    {
        ...
        { 
            ...
            if (trait.WasAdded(e))
                trait.Owner.OnInitiationPreReceived.Add(trait.GuidStrForEvents(0), OnOwnerInitiationPreReceived, PRIORITY);
            else if (trait.WasRemoved(e))
                trait.Owner.OnInitiationPreReceived.Remove(trait.GuidStrForEvents(0));
        }
        ...
        static async UniTask OnOwnerInitiationPreReceived(object sender, BattleInitiationRecvArgs rArgs)
        {
            ...
            await rArgs.strength.AdjustValueScale(-DAMAGE_REL_DECREASE * trait.GetStacks(), trait);
        }
    }
}
