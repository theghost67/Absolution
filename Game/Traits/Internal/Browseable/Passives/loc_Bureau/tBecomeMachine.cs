...
namespace Game.Traits
{
    ...
    public class tBecomeMachine : PassiveTrait
    {
        ...
        public override string DescRich(ITableTrait trait)
        {
            ...
            float strengthEffect = STRENGTH_REL_DECREASE_ON_OPPOSITE_SIDE_SEEN * 100;
            int moxieEffect = MOXIE_ABS_DECREASE_ON_OPPOSITE_SIDE_SEEN;
            return DescRichBase(trait, new TraitDescChunk[]
            {
                new($"При появлении карты <i>{cardName}</i> на вражеской стороне (П{PRIORITY})",
                    $"Уменьшает свою силу на {strengthEffect}% и инициативу на {moxieEffect} ед."),
            });
        }
